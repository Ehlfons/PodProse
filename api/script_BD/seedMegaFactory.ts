    ...
    for (let j = 0; j < usersPerCompany - 1; j++) {
      ...
      const user = await prisma.user.create({
        data: {
          ...
          DNI: generateRandomDNI(), // Genera un DNI aleatorio
          companyId: company.id,
          companyWorkdayId: generar1u2 ? dataWorkdayJornadaCompleta.id : dataWorkdayMediaJornada.id  ,
      ...
      await createVacationsForUser(user.id , generarVacacionesAleatorias( 2024 , generarNumeroAleatorioVacations() ) )
...
await createVacationsForUser(userSergio.id, generarVacacionesAleatorias( 2024 , generarNumeroAleatorioVacations() ));
